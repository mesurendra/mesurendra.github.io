...
In the above equation, partial derivative of the $E(w_i)$ w.r.t  $w_i$ give the direction in which weight should be changed to reduce the error. In the equation, the only computationally expensive operation is to compute partial derivative. Computing partial derivative is exponentially expensive when the number of layers of neural network increases. **Backpropagation is a cleaver and efficient way to compute partial derivative of error w.r.t. connection weight of the neural network**.
...
Figure 1 shows one of the simplest possible neural network which is good enough to understand details of the backpropagation algorithm. We consider input-layer as layer-1 ($$L_1$$), hidden-layer as ($$L_2$$) and output-layer as ($$L_3$$). $$ x_1$$  and $$ x_2$$ are input, blue nodes are nodes of hidden-layer and the red node is output-node. There are 6 connection weights $$ w_1$$ to $$ w_6$$. **In the training process we want to determine values of the connection weights which  minimize the square of error.**
...
The way to read $$ z_1^{(2)}$$ is z of  node-1 in layer-2 [**Note: subscript => item number and superscript => layer-number**] . $$ a_1^{(2)}$$ is the activation score of node-1 in layer-2. Following expressions can be obtained from figure 2 <br />
